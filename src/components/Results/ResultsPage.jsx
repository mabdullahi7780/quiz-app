...
function ResultsPage({ onPlayAgain, answers }) {
    const correctAnswers = answers.reduce((total, answerIdx, questionIdx) => {
        if (answerIdx === null) return total;
        return answerIdx === questionBank[questionIdx].correctIdx ? total + 1 : total;
    }, 0);

    const questionSummaries = questionBank.map((question, idx) => {
        ...
        return (
            <div
                ...
                className={['result-row', isCorrect ? 'result-row--correct' : 'result-row--incorrect']
                    .filter(Boolean)
                    .join(' ')}
            >
